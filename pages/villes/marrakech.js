...
export default function Marrakech({posts}) {
  const isAmp = useAmp()
  return (
    <Page>
      
      <PostList posts={posts} title="Marrakech" isAmp={isAmp}></PostList>
    </Page>
  );
...
posts = posts.filter(post => {return( post.title.toLowerCase().includes("marrakech") || post.preview.toLowerCase().includes("marrakech") ) })
//console.log("hnaaaa", posts)
